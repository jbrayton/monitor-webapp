...
class StackControllerTest < ActionController::TestCase
	...
	test "status_for_hash" do
		...
		assert_equal(:error, @controller.status_for_hash({"status" => "error", "ts" => Time.now.to_i + 5}))
		...
		assert_equal(:error, @controller.status_for_hash({"status" => "good"}))
		assert_equal(:error, @controller.status_for_hash({"status" => "good", "ts" => "xsf"}))
		...
		assert_equal(:error, @controller.status_for_hash({}))
		assert_equal(:error, @controller.status_for_hash(3))
		assert_equal(:error, @controller.status_for_hash(nil))
	end
